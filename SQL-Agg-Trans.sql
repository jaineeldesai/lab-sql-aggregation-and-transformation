...
SELECT *, MONTHNAME(rental_date) as month, DAYNAME(rental_date) as weekday -- MONTH() to show month number
FROM sakila.rental
LIMIT 20;
...
SELECT * ,
CASE
WHEN DAYNAME(rental_date)='Sunday' OR DAYNAME(rental_date)='Saturday' then 'Weekend'
ELSE 'Workday'
END AS 'DAY_TYPE'
FROM sakila.rental;
...
SELECT rating, SEC_TO_TIME(ROUND(AVG(length))*60) as mean_duration
FROM sakila.film
GROUP BY rating
ORDER BY mean_duration; 

-- HAVING mean_duration> '00:02:00'; 
-- not using HAVING as no ratings have mean greater than 2 hrs
